...
target_header = '''<div class="page-container">
          ...
          <div class="header-with-btn">
              <h1 style="color: #fff; margin: 0;">📊 일간 자산 추적 대시보드</h1>
              <button class="toggle-btn" onclick="toggleChart('trendChartContainer')">📈 변동 차트 보기</button>
          </div>'''
...
import re
content = re.sub(r'<div class="page-container">\s*<a href="/infinite" class="nav-link">.*?</button>\s*</div>', new_header, content, flags=re.DOTALL)
...
# Flexible replacement for chart_target since the label might be slightly different in HEAD
content = re.sub(r'// --- 1\. Total Asset Trend Chart ---.*?options:.*?ticks: {.*?callback: function\(value\) { return \'₩\' \+ value\.toLocaleString\(\); }.*?}.*?}.*?}.*?}\s*\);', chart_replacement, content, flags=re.DOTALL)

with open(r'C:\PycharmProjects\JamesWebServer\templates\infinite_assets.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("FINAL RESTORE SCRIPT FINISHED")
