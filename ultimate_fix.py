...
# 2. Add the Global Navigation & Broker Tabs (Replace the top header block)
...
# Flexible replace for header
import re
content = re.sub(r'<div class="page-container">\s*<a href="/infinite" class="nav-link">.*?</div>\s*</div>', new_header, content, flags=re.DOTALL)
...
# Replace all instances of ticker in table cells with the new layout
content = re.sub(
...
# 5. Fix Chart Logic
chart_pattern = r"// --- 1\. Total Asset Trend Chart ---.*?// --- 2\. Account Asset Distribution Pie Chart ---"
new_chart = '''// --- 1. Total Asset Trend Chart ---
                ...
                // --- 2. Account Asset Distribution Pie Chart ---'''
content = re.sub(chart_pattern, new_chart, content, flags=re.DOTALL)
