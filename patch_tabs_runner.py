...
target = '''<div class="page-container">
...
replacement = '''<div class="page-container">
        <!-- Global Navigation -->
        <div class="global-tabs">
            <a href="/infinite" class="global-tab active">🌐 자산</a>
            <a href="/infinite/macro" class="global-tab">🌍 거시 경제</a>
        </div>
        <!-- Broker Tabs -->
...
import re
content = re.sub(re.escape(target), replacement, content, flags=re.IGNORECASE|re.MULTILINE|re.DOTALL)
# Let's handle whitespace differences safely
target_compact = re.sub(r'\s+', ' ', target).strip()
content_compact = re.sub(r'\s+', ' ', content).strip()

if target_compact in content_compact:
    # Manual replacement since whitespace can be tricky
    start_idx = content_compact.find(target_compact)
    print("Found compact match!")
    
with open(r'C:\PycharmProjects\JamesWebServer\patch_tabs.py', 'w', encoding='utf-8') as script:
    script.write("""
import re
with open(r'C:\PycharmProjects\JamesWebServer\\templates\\infinite_assets.html', 'r', encoding='utf-8') as f:
    content = f.read()
